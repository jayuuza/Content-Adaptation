#463117 Jonathan Wasson
#good ai that generates data
...
#attempt at rank stacking algorithm
...
# def get_rank(heights):
# 	stack = [0]*9
# 	for i in range(len(stack)):
# 		stack[i] = heights[i+1]-heights[i]
# 		if (stack[i]>4):
# 			stack[i] = 4
# 		if (stack[i]<-4):
# 			stack[i] = 4
...
# def find_next_best_rank(board, piece, possiblerotations):
# 	largestRank = 0
# 	posX = 0
# 	posY = 0
# 	for r in range(possiblerotations):
# 		piece['rotation'] = possiblerotations
# 		for k in range(BOARDWIDTH-5):
# 			piece['y'] = k
# 			for l in range(BOARDHEIGHT-5):
# 				piece['x'] = l
# 				tempBoard = list(board)
# 				if (isValidPosition(tempBoard, piece)):
# 					for x in range(TEMPLATEWIDTH):
# 						for y in range(TEMPLATEHEIGHT):
# 							if PIECES[piece['shape']][piece['rotation']][y][x] != BLANK:
# 								tempBoard[x + piece['x']][y + piece['y']] = piece['color']


# 					heights = get_heights(tempBoard)
# 					rank = get_rank(heights)
# 					if (rank<largestRank):
# 						largestRank = rank
# 						posX = k
# 						posY = l
# 	return largestRank, posX, posY
...
def good_ai(board, currentBlock):
	...
	# 	if xcoords[n] < smallest:
	# 		smallest = xcoords[n]
	# 		posSmall = n

	# posY = xcoords[posSmall]
	# posX = smallest
