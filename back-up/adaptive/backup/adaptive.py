		...
		for event in pygame.event.get(): # event handling loop
			...
			if event.type == KEYUP:
				...
				if (event.key == K_LEFT or event.key == K_a) and isValidPosition(board, currentBlock, adjX=-1):
					...
					movingRight = False
					lastMoveSidewaysTime = time.time()
					change_probability(action,board,currentBlock)
...
def update_probabilities(probAgent, probAction):
	...
	probUp = float(probAction*probAgent)
	return probUp
...
def	change_probability(action,board,currentBlock):
	global MDPHUMAN
	global MDPBAD
	...
	#make appropriate changes to game
...
def write_prob():
	...
	with open("GameChanges.txt","a") as myfile:
		myfile.write(str(MDPPROBABILITIES[0]) + ',' + str(MDPPROBABILITIES[1]) + '\n')
